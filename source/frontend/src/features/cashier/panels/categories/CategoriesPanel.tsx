...
function CategoriesPanel({ permissions = [] }: CategoriesPanelProps) {
    ...
    const [categories, setCategories] = useState<Category[]>(apiService.getRealCategories());
    const [loading] = useState(false);
    const [showModal, setShowModal] = useState(false);
    const [editing, setEditing] = useState<Category | null>(null);
    const [formData, setFormData] = useState({ name: '', icon: 'coffee' });

    const reload = () => setCategories(apiService.getRealCategories());
    ...
    const handleSubmit = async () => {
        try {
            await apiService.saveCategory(editing ? editing.id : null, formData);
        } catch (e) {
            alert(e instanceof Error ? e.message : 'Gagal menyimpan kategori');
        }
        setShowModal(false);
        reload();
    };
