...
function UsersPanel({ permissions = [] }: UsersPanelProps) {
    ...
    const [users, setUsers] = useState<User[]>(mockService.getUsers());
    const [roles] = useState<Role[]>(mockService.getRoles());
    ...
    const reload = () => setUsers(mockService.getUsers());
    ...
    const handleSubmit = () => {
        if (editing) {
            console.log('Update user:', editing.id, formData);
        } else {
